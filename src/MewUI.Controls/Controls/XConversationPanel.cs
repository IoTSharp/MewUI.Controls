...
public class XConversationPanel : UserControl
{
    private readonly ObservableCollection<XMessage> _messages = new();
    private readonly ObservableCollection<XAttachment> _draftAttachments = new();
    ...
    public XConversationPanel()
    {
        Build();
    }

    public ObservableCollection<XMessage> Messages => _messages;

    public ObservableCollection<XAttachment> DraftAttachments => _draftAttachments;

    public event Func<string, IReadOnlyList<XAttachment>, Task<bool>>? SendRequested;

    public event Action? AttachRequested;

    public event Action? ConfigureRequested;

    public event Action<XAttachment>? DraftAttachmentRemoveRequested;
    ...
    public void SetMessages(IEnumerable<XMessage> messages)
    {
    ...
    public void SetDraftAttachments(IEnumerable<XAttachment> attachments)
    {
    ...
    public void AddDraftAttachment(XAttachment attachment)
    {
    ...
    public XMessage AddMessage(string role, string author, string content, IEnumerable<XAttachment>? attachments = null, IEnumerable<XContentBlock>? blocks = null)
    {
        ...
        {
            message.Blocks.Add(new XContentBlock { Kind = XContentBlockKind.Paragraph, Text = content });
        }
    ...
    protected override Element? OnBuild()
    {
        ...
        _messagesList = new ItemsControl()
            ...
            .ItemTemplate(new DelegateTemplate<XMessage>(
    ...
    private void BindMessageTemplate(FrameworkElement view, XMessage message, int index, TemplateContext ctx)
    {
    ...
    private static Element CreateContentBlock(XContentBlock block)
    {
        return block.Kind switch
        {
            XContentBlockKind.Code => new Border()
                .Padding(8)
                .BorderThickness(1)
                .CornerRadius(6)
                .WithTheme((t, border) =>
            ...
            XContentBlockKind.Quote => new Border()
                .Padding(8, 4)
                .BorderThickness(1)
                .CornerRadius(4)
                .WithTheme((t, border) =>
            ...
            XContentBlockKind.Canvas => block.Canvas ?? new RichCanvasPreview().Height(92),
    ...
    private static Element CreateAttachmentChip(XAttachment attachment)
    {
        return new Border()
            .Padding(6, 3)
            .BorderThickness(1)
    ...
    private async void SendDraft()
    {
        ...
        XAttachment[] attachments = _draftAttachments.ToArray();
    ...
    private async Task<bool> NotifySendRequestedAsync(string text, IReadOnlyList<XAttachment> attachments)
    {
        Func<string, IReadOnlyList<XAttachment>, Task<bool>>? handler = SendRequested;
        if (handler == null)
        {
            return true;
        }

        foreach (Func<string, IReadOnlyList<XAttachment>, Task<bool>> subscriber in handler.GetInvocationList())
        {
    ...
    private void RemoveDraftAttachment(XAttachment attachment)
    {
    ...
    private void OpenAttachment(XAttachment attachment)
    {
    ...
    private static IEnumerable<XContentBlock> ParseBlocks(string content)
    {
        ...
        foreach (var line in lines)
        {
            if (line.StartsWith("```", StringComparison.Ordinal))
            {
                if (inCode)
                {
                    yield return new XContentBlock { Kind = XContentBlockKind.Code, Text = string.Join('\n', code), Language = language };
                    code.Clear();
                    inCode = false;
                    language = null;
                }
            ...
            if (line.StartsWith("> ", StringComparison.Ordinal))
            {
                ...
                yield return new XContentBlock { Kind = XContentBlockKind.Quote, Text = line[2..] };
                continue;
            }
    ...
    private static IEnumerable<XContentBlock> FlushParagraph(List<string> paragraph)
    {
        ...
        yield return new XContentBlock { Kind = XContentBlockKind.Paragraph, Text = string.Join('\n', paragraph) };
        paragraph.Clear();
    }
