...
internal sealed class FramebufferInputPump : IDisposable
{
    ...
    private void ProcessEvent(EvdevTouchDevice.InputEvent ev)
    {
        ...
        DispatchTouch(point, touching, GetEventTimeMs(ev));
    }
    ...
    private void DispatchTouch(Point point, bool touching, long eventTimeMs)
    {
        if (_disposed)
        {
            return;
        ...
        if (touching == _lastDispatchedTouchDown)
        {
            ...
            if (throttleMs > 0)
            {
                if (eventTimeMs - _lastTouchMoveDispatchMs < throttleMs)
                {
                    return;
                }

                _lastTouchMoveDispatchMs = eventTimeMs;
            }
        }
        else
        {
