...
internal sealed class FramebufferWindowBackend : IWindowBackend
{
    ...
    private void ProcessPolledTouchEvent(EvdevTouchDevice device, EvdevTouchDevice.InputEvent ev)
    {
        if (_factory.Options.LogTouchInput)
        {
            Console.WriteLine($"[MewUI.Framebuffer] evdev type=0x{ev.type:X2} code=0x{ev.code:X3} value={ev.value}");
        }
        ...
        if (_factory.Options.LogTouchInput)
        {
            Console.WriteLine($"[MewUI.Framebuffer] touch raw=({_polledTouchState.RawX.Value},{_polledTouchState.RawY.Value}) mapped=({point.X:0.0},{point.Y:0.0}) down={touching}");
        }
        ...
        else
        {
            ...
            if (touching)
            {
                _lastPolledMoveDispatchMs = GetEventTimeMs(ev);
            }
        }
    ...
    private static long GetEventTimeMs(EvdevTouchDevice.InputEvent ev)
    {
        long seconds = ev.time_sec;
        long microseconds = ev.time_usec;
        if (seconds <= 0 && microseconds <= 0)
        {
            return Environment.TickCount64;
        }

        return checked(seconds * 1000 + microseconds / 1000);
    }
